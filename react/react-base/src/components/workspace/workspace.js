...
class WorkSpace extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevPath === thisPath && thisProps.params && thisProps.params.externals === true) {
      ...
      try {
        const AsyncAddComponent = dynamicWrapper(() => { return import(`../../pages/${thisProps.params.path}`) });
        ...
        if (_.startsWith(e.toString(), 'Error: Cannot find module')) {
          const AsyncNotFound = dynamicWrapper(() => { return import(`../../pages/${defaultPath}`) });
          // 打开notfound
      ...
      try {
        ...
        if (!apiPath) {
          const AsyncAddComponent = dynamicWrapper(() => { return import(`../../pages/${realPath}`) });
          this.add(realPath, this.getTitle(thisProps.transferedMap, realPath), AsyncAddComponent);
        } else {
          ...
          const AsyncAddComponent = dynamicWrapper(() => { return import(`../../pages/${apiPath}`) });
