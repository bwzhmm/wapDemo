...
// global.url['donate'] = window.global.ApiUrl;
/** Function : ajax方法封装修改 */
let startAjax = function (options) {
  return new Promise(function (resolve,reject) {
    if(options){
        let ajaxSettings = {
          // url:window.global.ApiUrl+options.url,
          url:options.url,
          method : 'POST',
          headers : {
...
/** Function : 登录页面  */
let redirectToLogin = function(res){
  let purl;
   // let purl=document.location.href.split("#")[0];
  console.log(window.global.isadmin)
  if(window.global.isadmin=='admin'){
    purl=document.location.href.split("#")[0]+"&authType="+window.global.authTypeadmin;//pc
  }else{
    purl=document.location.href.split("#")[0]+"&authType="+window.global.authType;//wx
  }
  
  console.log(purl)
  if (res!=null){
    location.href =window.global.ApiUrl+res.preUrl+purl;
  }
}
/** Function : 时间函数封装  */
...
//   }
// };
// var businessgetTicket = function(baseparams,path){
//   return new Promise(function (resolve,reject) {
//       util.postAjax({
...
//     var str = getRandomNum();
//     //生成签名
//     var signature = "jsapi_ticket=" + res.ticket + '&noncestr=' + str + '&timestamp=' + Date.parse(new Date()) + '&url=' + window.location.href;
//     wx.config({
//         debug: false, // 开启调试模式。
