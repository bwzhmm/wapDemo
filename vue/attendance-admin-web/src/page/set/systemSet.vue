<template>
  <div>
    ...
    <div class="systemBox">
      ...
      <el-form :model="setForm" ref="setForm" status-icon label-width="100px" class="demo-setForm">
        ...
        <el-form-item label="特殊日期：" :label-width="formLabelWidth">
          ...
          <el-table :data="needDateList" class="mg_tab">
            <el-table-column prop="DATE" label="日期" width="180">
              ...
              >{{formatDay(scope.row.DATE)}}</template>
            </el-table-column>
    ...
    <el-dialog title="年假扣除" :visible.sync="annualVisible" width="600px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleCancelAnnual">取消</el-button>
...
export default {
  data() {
    return {
      ...
      setForm: {
        CFG: {
          YEARCLEANDAY: "1203", // 年假清零日期
          STARTTIME: "10:35", // 上班时间
      ...
      initSetList: {
        CFG: {
          YEARCLEANDAY: "1204", // 年假清零日期
          STARTTIME: "", // 上班时间
          STARTTIME_LATEST: "" // 最迟打卡时间
        },
        YEAR: [],
        needDate: "",
        noNeedDate: "",
  ...
  methods: {
    formatDay(data) {
      if (Array.isArray(data)) {
        ...
        data = util.formatTime(data, "YYYY-MM-DD");
      }
      return data;
    },
    ...
    getSetList() {
      fetchSets().then(res => {
        ...
        // 格式化为相应格式
        setData.CFG.YEARCLEANDAY = stringDay(setData.CFG.YEARCLEANDAY);
