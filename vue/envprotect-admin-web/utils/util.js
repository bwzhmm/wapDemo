...
global.url['business'] = window.global.ApiUrl;
// global.url['login'] = window.global.loginApiUrl;
    ...
    if (options && options.url && options.code) {
      getRealUrl(options.code, options.url).then(function (data) {
        let ajaxSettings = {
          ...
          method: 'POST',
          headers: {
            "IDSTGC": getCookie("IDSTGC") || '1156dfaf16664622afaeca1a331ccbba',
          },
...
/** Function : 登录页面 , Created By HarryC on 2019/1/31 0031  跳转到自己的登录页默认路由*/
let redirectToLogin = function (res) {
  let purl = document.location.href.split("#")[0];
  if (res != null) {
    // let herfUrl = window.global.redirectToUrl + res.preUrl + encodeURIComponent(purl);
