<template>
  <div>
    <div class="info-box">
      ...
      <span class="map-title" @click="clickMap(fromInfo)">
        <i class="iconfont iconshezhi">GIS地图</i>
      </span>
    </div>
    ...
    <div class="base-info">
      <span class="mg-r10">
        <el-image
          ...
          class="img-style"
          :src="imgURL"
          @contextmenu.prevent="openImgViewer(index)"
      ...
      <div>
        <div class="rflex info-title">
          <h1 class="h1-title">{{fromInfo.NAME}}</h1>
          <el-button @click="handleEdit" type="primary" size="medium">
            <i class="iconfont iconbtn-bianji"></i> 编辑
          </el-button>
        </div>
        <el-form :model="fromInfo" ref="fromInfo" label-position="left">
...
export default {
  ...
  methods: {
    handleEdit() {
      let row = this.fromInfo;
      this.$emit("jumperEdit", row);
    },
    clickMap(fromInfo) {
      this.$router.push({
        name: "enterprise-map",
        params: { company: fromInfo }
      });
    }
  }
};
