  ...
  <div>
    ...
    <div class="table-container" v-if="tableData.length">
      <el-table class="my-table" :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="立项文件" align="center">
          <template slot-scope="scope">
...
import { projectCol, projectDrawerCol } from "../tool.js";
export default {
  components: {
    updateDrawer,
    lookDrawer,
  ...
  data() {
    return {
      ...
      projectDrawerCol: projectDrawerCol,
      drawerTitle: "",
      ENVPATH: "ENV_PROJECTINFO",
      fromData: {}
    };
  },
  mounted() {
  ...
  methods: {
    ...
    handleUpdate(row, isLook) {
      ...
      this.drawerTitle =
        row && row.ID ? (isLook ? "查看立项" : "编辑立项") : "新增立项";
      if (row && row.ID) {
    ...
    getTableList() {
      ...
      service(params, path).then(res => {
        ...
        if (res.success) {
          ...
          itemsArr.map(
            item => (item.CREATETIME = formatStr(item.CREATETIME, "-"))
          );
