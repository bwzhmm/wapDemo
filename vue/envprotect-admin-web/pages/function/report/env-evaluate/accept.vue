  ...
  <div>
    ...
    <div class="table-container" v-if="tableData.length">
      <el-table class="my-table" :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="批复意见" align="center">
          <template slot-scope="scope">
...
import { acceptCol, acceptDrawerCol } from "../tool.js";
export default {
  components: {
    updateDrawer,
    lookDrawer,
  ...
  data() {
    return {
      ...
      acceptDrawerCol: acceptDrawerCol,
      drawerTitle: "",
      ENVPATH: "ENV_ACCEPT",
      fromData: {}
    };
  ...
  methods: {
    ...
    handleUpdate(row, isLook) {
      ...
      this.drawerTitle =
        row && row.ID ? (isLook ? "查看验收" : "编辑验收") : "新增验收";
      if (row && row.ID) {
